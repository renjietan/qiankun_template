...
<template>
  <div style="padding-bottom: 20px;">
      ...
      <el-row :gutter="40">
        ...
        <el-col :span="10" :offset="10" style="text-align: right; height: 96px;">
          <div style="margin-top: 45px;">
            <el-button icon="el-icon-pinggu" v-waves type="success">评估结果确认书</el-button>
            <el-button icon="el-icon-biaoge" v-waves type="warning">能力评估表</el-button>
            <el-button icon="el-icon-dayin" v-waves type="primary">一键打印</el-button>
          </div>
        </el-col>
      </el-row>
    </el-form>
    <div>
      <el-table border ref="data_table" :data="table_data" v-loading="table_loading" fit>
        <el-table-column width="200" align="center" prop="type" label="评估项目">
          <template
            slot-scope="scope">{{scope.row.type=="AC"?"日常生活活动":scope.row.type=="SP"?"精神状态":scope.row.type=="SC"?"感知觉与沟通":"社会参与"}}</template>
        </el-table-column>
        ...
        <el-col :span="10">
          <div class="binding-content">
            ...
            <div class="binding-card" style="margin:40px">
              ...
              <div class="paragraph">（1）当基础性日常生活自理能力为0时，认知能力、感知觉与沟通有一项为一及以上，或者工具性日常生活自理能力为2或以上，判定为轻度失能。</div>
    ...
    <div class="footer">
      <el-button type="primary" :disabled="step >= 7 ? 'disabled' : false" v-waves @click="handleSubmit">提交</el-button>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      getAassessinfoAll() {
        get_assessinfo_all({ elderId: this.$route.params.id }).then(res => {
