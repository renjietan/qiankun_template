  ...
  <div class="">
    <el-form :inline="true" ref="form" :model="form_data_baseInfo" hide-required-asterisk status-icon>
      <el-row :gutter="40">
        ...
        <el-col :span="4">
          <el-form-item class="readonly" label="合同类型">
            <el-input v-model="form_data_baseInfo.contract_type"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="4">
          <el-form-item class="readonly" label="调整愿意">
            <el-input v-model="form_data_baseInfo.chargeReason"></el-input>
          </el-form-item>
        </el-col>
    ...
    <div style="padding: 20px 0">
      <el-button type="primary" @click="handleDialog('', '添加周期付款计划')" icon="el-icon-plus">添加付款计划</el-button>
    ...
    <el-table border :row-class-name="tableRowClassName" :span-method="objectSpanMethod" ref="data_table"
      ...
      <el-table-column width="200" align="center" prop="cycle" label="时间周期"></el-table-column>
      ...
      <el-table-column align="center" label="操作" width="200">
        <template slot-scope="scope">
          <div>
            <el-link :underline="false" type="primary" @click="handleDialog(scope.row, '周期费用修改')">修改</el-link>
    ...
    <div class="footer">
      <el-button :disabled="cur_step < 2 ? false : 'disabled'" @click="handleComplete" type="primary">确认付款计划</el-button>
    </div>
  </div>
</template>
<script>
  ...
  import { get_inLive_payPlan, get_charge_process_byId_byType, confirm_pay_plan2 } from '@/api/pensionagency/inLive/server_level';
    ...
    methods: {
      handleComplete() {
        ...
        let params = {
          ...
          type: this.form_data_baseInfo.type,
          chargeReason: this.form_data_baseInfo.chargeReason,
        }
        confirm_pay_plan2(params).then((res) => {
          this.cur_step = 2
          this.$notify({
            type: "success",
            title: "操作",
      ...
      async getDetail() {
        ...
        if (res.id != null) {
          self.pay_plan_id = res.id
          self.form_data_baseInfo.chargeReason = res.chargeReason
      ...
      getInfo() {
        ...
        base_info({ id }).then((res) => {
          if (res.id != null) {
            ...
            this.form_data.feeDesc = res.feeDesc;
            this.form_data.other = res.other;
            this.form_data.afterName = res.afterName;
            this.form_data.expectDate = res.expectDate;
            this.form_data.applyDate = res.applyDate;
          }
        });
      },
    },
