<!-- 
 * @description:增值服务类型 
 * @fileName: service_added_type_mg.vue 
 * @author: 姜纬杰 
 * @date: 2021-07-31 13:56:23
 * @后台人员:  
!-->
<template>
  <div class="container">
    <div class="container_card">
      <el-row style="line-height: 80px;margin-top: -30px;">
        <el-col :span="6">
          <div class="title">增值服务类型列表</div>
        </el-col>
        <el-col :span="18" style="text-align: right;">
          <el-button type="primary" icon="el-icon-plus" @click="handleDialog('添加增值服务类型')">添加增值服务类型</el-button>
        </el-col>
      </el-row>
      <el-table border ref="data_table" :data="table_data" v-loading="table_loading" fit>
        ...
        <el-table-column width="200" align="center" prop="avType" label="增值服务类型"></el-table-column>

        <el-table-column align="center" prop="rsv" label="说明"></el-table-column>
        <el-table-column width="200" align="center" label="操作">
          <template slot-scope="scope">
            <el-link type="primary" @click="handleDialog('编辑增值服务类型',scope)" :underline="false">编辑</el-link>
    ...
    >
      ...
      >
        <el-row>
          <el-col :span="11">
            <el-form-item class="must-fill" label="增值服务名称" prop="avType">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          @click="dialog_cof.title == '编辑增值服务类型' ? handleSubmit_edit() : handleSubmit_create()"
        >确 认</el-button>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleDialog(title, row) {
      ...
      if (self.dialog_cof.title == "编辑增值服务类型") {
        let data = row.row;
        self.form_data = {
          id: data.id,
          avType: data.avType,
