        ...
        <template slot-scope="scope">
          ...
          >
            ...
            <el-link
              ...
              ><i class="el-icon-edit"></i>修改</el-link
            >
            <el-link
              :disabled="step >= 7 ? 'disabled' : false"
              :underline="false"
  ...
  methods: {
    ...
    getDetail() {
      Promise.all([pay_type(), one_time_type()]).then((res) => {
        ...
        console.log(this.fee_name_list);
    ...
    handleDialog(row, index, title) {
      ...
      switch (title) {
        ...
        case "添加一次性付款计划":
          ...
          break;
        case "周期费用修改":
          if (self.update_data && self.update_data?.id) {
            self.reset();
            self.disabled = true;
            self.form_data.tollType = "周期费用";
            self.form_data.planName = self.update_data.planType;
            self.form_data.id = self.update_data.id;
            self.form_data.startDate = self.update_data.cycle.split("~")[0];
            self.form_data.endDate = self.update_data.cycle.split("~")[1];
            self.form_data.tollStandard = self.update_data.tollStandard;
            self.form_data.afterDiscount = self.update_data.afterDiscount;
            self.form_data.rsv = self.update_data.rsv;
            self.dialog_cof.visible = true;
          } else {
            self.$notify({
              type: "warning",
        ...
        case "一次性费用修改":
          ...
          self.form_data.planName = row.planType;
