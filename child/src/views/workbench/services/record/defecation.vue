<!-- 
 * @description: 排泄记录表
 * @fileName: services_record_defecation.vue 
 * @author: 谭人杰 
...
<template>
    <div class='container'>
        <div class="container_card">
            <div class="title" style="margin-bottom: 20px;">排泄记录表</div>
    ...
    export default {
        ...
        methods: {
            ...
            async init() {
                ...
                times = times?.map(item => item.projectName) ?? []
                let r = res?.result ?? []
                let table_data = []
                let d = []
                ...
                for (let i of r) {
                    ...
                    for (let item of t) {
                        ...
                        for (let child of s) {
                            let c = { ...child }
                            c.serviceDate = this.Calendar_obj.format(new Date(obj.serviceDate))
                            c.elderId = obj.elderId
                            d.push(c)
                ...
                table_data = table_data.map(item => {
                    ...
                    t.forEach(el => {
                        ...
                        let times = s.time ?? []
                        let res = times.filter(item => item.prop.indexOf(el.serviceDate) >= 0 && item.columnName == el.projectName)[0] ?? {}
                        item[res.prop] = el.times
                    })
