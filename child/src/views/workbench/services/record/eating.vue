<!-- 
 * @description: 翻身记录表
 * @fileName: services_record_eating.vue 
 * @author: 谭人杰 
                ...
                for (let i of r) {
                    ...
                    for (let item of t) {
                        ...
                        for(let child of s) {
                            ...
                            let sum = child.timeGroups?.reduce((cur,pre) => {
                                cur = cur + Number(pre.data)
                                return cur
                            }, 0)
                            obj[`${this.Calendar_obj.format(new Date(obj.serviceDate))}（${temp_index}）`] = sum
                        }
                        // s = s[0]?.data
