...
<template>
  <div style="padding-bottom: 20px;">
      ...
      <el-row>
        <el-col :span="6">
          <el-form-item class="readonly" label="评分总和">
            <el-input v-model="totalScore.total" :disabled="true" required placeholder="请输入评分总和"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="17" :offset="1">
          <el-form-item class="default" label="判断等级">
            <el-checkbox-group v-model="level_checked" :max="1">
              <el-checkbox  v-for="item in level" :key="item.value" :label="item.label" :value="item.value">
              </el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </el-col>
    ...
    <el-table border :span-method="objectSpanMethod" ref="data_table" :data="table_data" v-loading="table_loading" fit>
      <el-table-column width="150" align="left" prop="projectName" label="项目名称"></el-table-column>
      ...
      <el-table-column width="100" align="center" label="评分">
        <template slot-scope="scope">
          <el-checkbox @change="handleChange(scope)"
            :dsiabled="checkList_model[scope.row.projectName][scope.row.keys].isDisabled"
            v-model="checkList_model[scope.row.projectName][scope.row.keys].isChecked" :disabled="scope.row.disabled">
          </el-checkbox>
        </template>
      </el-table-column>
    ...
    <div class="footer">
      <el-button :disabled="step >= 7 ? 'disabled' : false" type="primary" v-waves @click="handleSubmit">保存</el-button>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    computed: {
      level_checked() {
        let len = Object.keys(this.totalScore).length
        let ys = this.totalScore["意识"] ?? 0
        let sl = this.totalScore["视力"] ?? 0
        let tl = this.totalScore["听力"] ?? 0
        let gt = this.totalScore["沟通"] ?? 0
        let res = []
        if (ys == 2 || ys == 3 || gt == 3 || sl == 4 || tl == 4) {
        ...
        if (ys == 0 && gt == 0) {
          ...
          } else if ((sl == 2 && tl < 2) || (sl < 2 && tl == 2)) {
            return [this.level[1].label]
          } else if ((sl == 3 && tl < 3) || (sl < 3 && tl == 3)) {
      ...
      totalScore() {
        let flt_arr = Object.values(this.checkList_model)
          .flat()
          .filter(item => item.isChecked).reduce((cur, pre) => {
            if (pre.projectName.indexOf("意识") >= 0) {
              cur["意识"] = pre.point
            } else if (pre.projectName.indexOf("视力") >= 0) {
              cur["视力"] = pre.point
            } else if (pre.projectName.indexOf("听力") >= 0) {
              cur["听力"] = pre.point
            } else {
              cur["沟通"] = pre.point
            }
            cur.total = cur.total + pre.point
            return cur
          }, { total: 0 })
        return flt_arr;
      }
    ...
    methods: {
      ...
      handleChange(scope) {
        let { projectName, keys, id } = scope.row;
      ...
      getPerceptionConfig() {
        let self = this;
        get_ability_assess_config({ type: self.type }).then(res => {
      ...
      arrayGroupBy(arr, key) {
        ...
        let newlist = arr.reduce((cur, pre, curIndex) => {
          if (cur[pre[key]] == undefined) {
            ...
            self.$set(self.checkList_model[pre[key]][0], "projectName", pre.projectName);
          } else {
