...
<template>
    <div class='container'>
        <div class="container_card">
            <div class="title">
                <span>床位费管理</span>
                <el-button v-waves type="primary" icon="el-icon-plus" @click="handleDialog('添加床位费')">添加床位费</el-button>
            </div>
            <el-table  ref="data_table" :data="table_data" v-loading="table_loading" stripe
                ...
                <el-table-column width=”100“ align="center" label="床位费">
                    <template slot-scope="scope">
                        <div>
                            {{ scope.row.fee }} / 月
                        </div>
                ...
                <el-table-column align="center"  label="操作">
                    <template slot-scope="scope">
                        <el-link type="primary" @click="handleDialog('编辑床位费', scope.row)" :underline="false">编辑
                        </el-link>
            ...
            <el-dialog class="el-dialog-2" center v-el-drag-dialog :visible.sync='dialog_cof.visible'
                ...
                <el-form :inline="true" ref="form" :model="form_data" :rules="rules" hide-required-asterisk status-icon>
                    <el-row>
                        <el-col :span="13">
                            <el-form-item class="must-fill" label="床位费" prop="fee">
        ...
        methods: {
            ...
            handleSubmit() {
                ...
                self.$refs.form.validate(valid => {
                    if (valid) {
                        if (self.dialog_cof.title == "编辑床位费") {
                            let params = { ...self.form_data, ...{ operateType: handleType["编辑"] } }
